...
def find_nearest_node(req_start, locations):
	lat1, lng1 = radians(req_start[0]), radians(req_start[1])
	distances = []
	for i in range(len(locations)):
		lat2, lng2 = radians(locations[i]["lat"]), radians(locations[i]["lng"])
		dlng = lng2 - lng1
		dlat = lat2 - lat1
		a = sin(dlat / 2)**2 + cos(lat1) * cos(lat2) * sin(dlng / 2)**2
		c = 2 * atan2(sqrt(a), sqrt(1 - a))
		distance = c * 6371 * 1000  # convert to meters
		distances.append(distance)
	return distances.index(min(distances))


def aco_shortest_path_geo(locations, req_start, req_goal):
	...
	# this line is difference from that aco_shortest_path
	start_state = find_nearest_node(req_start, locations)
